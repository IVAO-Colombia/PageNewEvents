    ...
    <table class="min-w-full text-xs md:text-sm">
        ...
        <thead>
            <tr class="text-center border-b dark:border-gray-700">
                ...
                @php
                $startTime = \Carbon\Carbon::parse($this->event->start_time);
                $endTime = \Carbon\Carbon::parse($this->event->end_time);
                ...
                @for ($hour = $startHour; $hour <= $endHour; $hour++) <th class="py-3 px-2 bg-gray-800 text-white">
                    {{ sprintf('%02d:00', $hour) }}
                    </th>
                    @endfor
            </tr>
        ...
        <tbody>
            @php
            $searchPositions = app(\App\Services\Ivao::class)->getPositionsAtc($this->event->icao);
            $positions = [];
            ...
            <tr class="border-b dark:border-gray-700">
                ...
                @for ($hour = $startHour; $hour <= $endHour; $hour++) @php $slotDateTime=$startTime->
                    ...
                    <td class="py-2 px-1">
                        ...
                        @else
                        <button type="button"
                            class="w-full bg-gray-400 cursor-not-allowed text-white py-2 px-1 rounded text-xs" disabled>
                            {{ __('Occupied') }}
                        </button>
                        @endif
                    </td>
